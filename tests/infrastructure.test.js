...
test("activation removes only previous Invoice Studio caches", async () => {
  ...
  await activation;
  assert.deepEqual(worker.deletedCaches, ["invoice-studio-v1", "invoice-studio-v27", "invoice-studio-v28", "invoice-studio-v29"]);
...
test("only named-cache shell requests are cached and used offline", async () => {
  const shellUrl = `${SCOPE}app.js?v=30`;
  ...
  assert.deepEqual(worker.cachePuts, [{ cacheName: "invoice-studio-v30", key: shellUrl }]);
